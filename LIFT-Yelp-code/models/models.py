...
class PeftModelFromCLIP(nn.Module):
    ...
    def forward(self, image, use_tuner=True, return_feature=False, labels=None):
        ...
        if isinstance(self.image_encoder, Peft_ViT):
            return self.image_encoder(image, tuner, head, labels=labels)
...
class PeftTextModelFromCLIP(nn.Module):
    def __init__(self, cfg, clip_model, num_classes):
        super().__init__()

        self.text_encoder = Peft_Text(clip_model)
        self.tuner = Text_Tuner(cfg, clip_model, num_classes)

        feat_dim = self.text_encoder.out_dim
        dtype = self.text_encoder.dtype
        self.head = eval(cfg.classifier)(feat_dim, num_classes, dtype, **cfg)

    def encode_text(self, text, use_tuner=False):
        tuner = self.tuner if use_tuner else None
        return self.text_encoder(text, tuner=tuner, head=None)

    def forward(self, text, use_tuner=True, return_feature=False):
        tuner = self.tuner if use_tuner else None
        head = self.head if not return_feature else None
        return self.text_encoder(text, tuner=tuner, head=head)
