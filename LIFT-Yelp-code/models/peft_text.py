...
class Text_Tuner(nn.Module):
    ...
    def __init__(self, cfg, clip_model, num_classes):
        ...
        seq_len = clip_model.positional_embedding.shape[0]
        ...
        if (use_vpt_shallow or use_vpt_deep) and (vpt_len is None):
            vpt_len = 10
            print("Text prompt length set to {}".format(vpt_len))
        ...
        if use_ln_tuning:
            ln_tuned = nn.ModuleList([
                mod for _, mod in blocks.named_modules()
        ...
        if use_vpt_shallow:
            vpt_list = nn.ModuleList([
                VPT(vpt_len=vpt_len, seq_len=seq_len, patch_size=fake_patch_size, emb_dim=emb_dim, dtype=dtype),
                *[None] * (n_layers - 1)
            ])
        elif use_vpt_deep:
            vpt_list = nn.ModuleList([
...
class Peft_Text(nn.Module):
    def __init__(self, clip_model):
        ...
        self.out_dim = self.text_projection.shape[1]

    @property
    def dtype(self):
        return self.token_embedding.weight.dtype

    def forward(self, text, tuner=None, head=None):
