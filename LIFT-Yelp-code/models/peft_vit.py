...
class ViT_Tuner(nn.Module):
    ...
    def __init__(self, cfg, vit_model, num_classes):
        ...
        use_tmr_lora = cfg.tmr_lora
        use_tmr_lora_mlp = cfg.tmr_lora_mlp
        ...
        partial = cfg.partial
        vpt_len = cfg.vpt_len
        adapter_dim = cfg.lora_rank if cfg.lora_rank is not None else cfg.adapter_dim
        ...
        if (use_adapter or use_adaptformer or use_lora or use_lora_mlp or use_tmr_lora or use_tmr_lora_mlp) and (adapter_dim is None):
        ...
        if use_lora:
            lora_list = nn.ModuleList([
                *[None] * (_start),
                *[nn.ModuleDict({
                    "q": LoRA(in_dim=emb_dim, bottle_dim=adapter_dim, dtype=dtype, alpha=cfg.lora_alpha, dropout=cfg.lora_dropout),
                    "v": LoRA(in_dim=emb_dim, bottle_dim=adapter_dim, dtype=dtype, alpha=cfg.lora_alpha, dropout=cfg.lora_dropout),
        ...
        if use_lora_mlp:
            lora_mlp_list = nn.ModuleList([
                *[None] * (_start),
                *[nn.ModuleDict({
                    "1": LoRA(in_dim=emb_dim, bottle_dim=adapter_dim, out_dim=mlp_in_dim, dtype=dtype, alpha=cfg.lora_alpha, dropout=cfg.lora_dropout),
                    "2": LoRA(in_dim=mlp_in_dim, bottle_dim=adapter_dim, out_dim=emb_dim, dtype=dtype, alpha=cfg.lora_alpha, dropout=cfg.lora_dropout),
                }) for _ in range(_start, _end)],
                *[None] * (n_layers - _end)
            ])
        else:
        ...
        if use_tmr_lora:
            tmr_lora_list = nn.ModuleList([
                ...
                *[nn.ModuleDict({
                    "q": TMRLoRA(
                        ...
                        num_experts=cfg.tmr_num_experts,
                        routing=cfg.tmr_routing,
                        dtype=dtype,
                        head_label_ids=cfg.tmr_head_labels,
                        tail_label_ids=cfg.tmr_tail_labels,
                        oracle_head_weights=cfg.tmr_oracle_head_weights,
                        oracle_tail_weights=cfg.tmr_oracle_tail_weights,
                        batch_dim=1,
                    ),
                    "v": TMRLoRA(
                        in_dim=emb_dim,
            ...
            tmr_lora_list = nn.ModuleList([None] * n_layers)

        if use_tmr_lora_mlp:
            tmr_lora_mlp_list = nn.ModuleList([
                *[None] * (_start),
                *[nn.ModuleDict({
                    "1": TMRLoRA(
                        in_dim=emb_dim,
            ...
            tmr_lora_mlp_list = nn.ModuleList([None] * n_layers)
        ...
        self.tmr_lora_list = tmr_lora_list
        self.tmr_lora_mlp_list = tmr_lora_mlp_list
...
class Peft_ViT(nn.Module):
    ...
    def forward(self, x, tuner=None, head=None, labels=None):
        x = x.to(self.dtype)
        ...
        for i in range(n_layers):
            ...
            if tuner is not None:
                ...
                tmr_lora = tuner.tmr_lora_list[i]
                tmr_lora_mlp = tuner.tmr_lora_mlp_list[i]
                ...
                vpt = adapter = adaptformer = lora = lora_mlp = tmr_lora = tmr_lora_mlp = ssf_attn = ssf_mlp = ssf_ln = masked_linear = None
            ...
            if tmr_lora is not None:
                q = q + tmr_lora["q"](x, labels=labels)
                v = v + tmr_lora["v"](x, labels=labels)
            ...
            if tmr_lora_mlp is not None:
                x_out = x_out + tmr_lora_mlp["1"](x, labels=labels)
            ...
            if tmr_lora_mlp is not None:
                x_out = x_out + tmr_lora_mlp["2"](x, labels=labels)
